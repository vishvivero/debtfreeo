...
export const AddDebtForm = ({
  ...
  useEffect(() => {
    if (isInterestIncluded && remainingMonths && balance && minimumPayment && interestRate) {
      const principal = InterestCalculator.calculatePrincipalFromTotal(Number(balance), Number(interestRate), Number(minimumPayment), Number(remainingMonths));
  ...
  const estimatedInterestRate = useRemainingMonths && balance && minimumPayment && remainingMonths ? calculateEstimatedInterestRate(Number(balance), Number(minimumPayment), Number(remainingMonths)) : null;
  const handleSubmit = async (e: React.FormEvent<HTMLFormElement>) => {
    ...
    try {
      ...
      const finalBalance = usePrincipalAsBalance && calculatedPrincipal !== null ? calculatedPrincipal : Number(balance);
      const newDebt = {
        name,
  ...
  function calculateEstimatedInterestRate(principal: number, monthlyPayment: number, months: number): number | null {
  ...
  return <form onSubmit={handleSubmit} className="flex flex-col">
          ...
          <div className="border-b">
            <div className="px-4">
              <TabsList className="h-12 bg-transparent space-x-5 p-0">
                <TabsTrigger value="basics" className="text-sm font-medium border-b-2 border-transparent data-[state=active]:border-blue-500 data-[state=active]:text-blue-600 rounded-none h-12 px-1 bg-transparent">
                  Basic Info
                </TabsTrigger>
                <TabsTrigger value="advanced" className="text-sm font-medium border-b-2 border-transparent data-[state=active]:border-blue-500 data-[state=active]:text-blue-600 rounded-none h-12 px-1 bg-transparent">
                  Advanced Settings
                </TabsTrigger>
                <TabsTrigger value="currency" className="text-sm font-medium border-b-2 border-transparent data-[state=active]:border-blue-500 data-[state=active]:text-blue-600 rounded-none h-12 px-1 bg-transparent">
                  Currency
                </TabsTrigger>
              </TabsList>
            </div>
                ...
                <div className="space-y-3">
                  ...
                  <div className="space-y-1">
                    ...
                    <div className="relative">
                      ...
                      <Input value={name} onChange={e => setName(e.target.value)} className="pl-9 border-gray-300 h-9" placeholder="E.g. Personal Loan, Credit Card" required />
                    </div>
                  </div>
                  ...
                  <div className="space-y-1">
                    ...
                    <div className="relative">
                      ...
                      <Input type="number" value={balance} onChange={e => setBalance(e.target.value)} className="pl-9 border-gray-300 h-9" placeholder="Enter balance amount" required min="0" step="0.01" />
                    </div>
                  </div>
                ...
                <div className="space-y-3">
                  ...
                  <div className="space-y-1">
                    ...
                    <div className="relative">
                      ...
                      <Input type="number" value={interestRate} onChange={e => setInterestRate(e.target.value)} className="pl-9 border-gray-300 h-9" placeholder="E.g. 5.99" required={!useRemainingMonths} disabled={useRemainingMonths} min="0" max="100" step="0.01" />
                    </div>
                  </div>
                  ...
                  <div className="space-y-1">
                    ...
                    <div className="relative">
                      ...
                      <Input type="number" value={minimumPayment} onChange={e => setMinimumPayment(e.target.value)} className="pl-9 border-gray-300 h-9" placeholder="Monthly payment" required min="0" step="0.01" />
                    </div>
                  </div>
                  ...
                  <div className="space-y-1">
                    ...
                    <div className="relative">
                      <Input type="date" value={formatDateForInput(date)} onChange={e => {
                      if (e.target.valueAsDate) {
                        setDate(e.target.valueAsDate);
                      }
                    }} className="border-gray-300 h-9 pl-9" min={formatDateForInput(new Date())} required />
                      <div className="absolute inset-y-0 left-3 flex items-center pointer-events-none">
            ...
            <TabsContent value="advanced" className="mt-0 space-y-3">
              <div className="space-y-4">
                ...
                <div className="p-3 border rounded-md bg-gray-50">
                  <div className="flex justify-between items-start mb-2">
                    ...
                    <div className="flex items-center">
                      ...
                      <div className="relative inline-flex h-5 w-10 items-center rounded-full bg-gray-300 transition-colors duration-200 ease-in-out focus-within:ring-2 focus-within:ring-offset-2 focus-within:ring-offset-white data-[state=checked]:bg-blue-500">
                        <input type="checkbox" checked={isInterestIncluded} onChange={e => {
                  ...
                  {isInterestIncluded && <div className="mt-3">
                      <Label className="text-xs font-medium">Months Until Payoff</Label>
                      <Input type="number" value={remainingMonths} onChange={e => setRemainingMonths(e.target.value)} placeholder="36" className="mt-1 text-xs h-8" min="1" required={isInterestIncluded} />
                      
                      {balance && minimumPayment && remainingMonths && interestRate && <div className="mt-2 p-2 bg-blue-50 border border-blue-100 rounded-md text-xs">
                          <div className="grid grid-cols-2 gap-x-2 gap-y-1">
                            {calculatedPrincipal !== null && <>
                                <div>
                                  <p className="text-xs font-medium text-blue-800">Principal:</p>
                                  <p className="text-xs text-blue-600">
                                    {selectedCurrency}{calculatedPrincipal.toLocaleString(undefined, {
                              maximumFractionDigits: 2
                                ...
                                <div>
                                  ...
                                  <p className="text-xs text-blue-600">
                                    {selectedCurrency}{(Number(balance) - calculatedPrincipal).toLocaleString(undefined, {
                              maximumFractionDigits: 2
                            })}
                                  </p>
                                </div>
                          ...
                          <div className="mt-2 flex items-center justify-between">
                            ...
                            <div className="relative inline-flex h-5 w-10 items-center rounded-full bg-gray-300 transition-colors duration-200 ease-in-out focus-within:ring-2 focus-within:ring-offset-2 focus-within:ring-offset-white data-[state=checked]:bg-blue-500">
                              <input type="checkbox" checked={usePrincipalAsBalance} onChange={e => setUsePrincipalAsBalance(e.target.checked)} className="peer absolute h-0 w-0 opacity-0" />
                              <span className="pointer-events-none block h-4 w-4 rounded-full bg-white transition-transform duration-200 ease-in-out peer-checked:translate-x-5" />
                            </div>
                          </div>
                        </div>}
                ...
                <div className="p-3 border rounded-md bg-gray-50">
                  <div className="flex justify-between items-start mb-2">
                    ...
                    <div className="flex items-center">
                      ...
                      <div className="relative inline-flex h-5 w-10 items-center rounded-full bg-gray-300 transition-colors duration-200 ease-in-out focus-within:ring-2 focus-within:ring-offset-2 focus-within:ring-offset-white data-[state=checked]:bg-blue-500">
                        <input type="checkbox" checked={useRemainingMonths} onChange={e => {
                  ...
                  {useRemainingMonths && <div className="mt-3">
                      <Label className="text-xs font-medium">Months Until Payoff</Label>
                      <Input type="number" value={remainingMonths} onChange={e => setRemainingMonths(e.target.value)} placeholder="36" className="mt-1 text-xs h-8" min="1" required={useRemainingMonths} />
                      
                      {balance && minimumPayment && remainingMonths && <div className="mt-2 p-2 bg-blue-50 border border-blue-100 rounded-md text-xs">
                          <div className="grid grid-cols-2 gap-1">
                            ...
                            {estimatedInterestRate !== null && <div>
                ...
                <div className="space-y-1">
                  ...
                  <textarea value={notes} onChange={e => setNotes(e.target.value)} className="w-full border border-gray-300 rounded-md p-2 h-20 resize-none text-sm focus:ring-2 focus:ring-blue-500 focus:border-transparent" placeholder="Add any additional notes about this debt..." maxLength={500} />
                  ...
                  <div className="space-y-1">
                    ...
                    <Select value={selectedCurrency} onValueChange={setSelectedCurrency}>
                      ...
                      <SelectContent className="max-h-[200px]">
                        {countryCurrencies.map(item => <SelectItem key={item.symbol} value={item.symbol}>
      ...
      <div className="mt-auto flex justify-between items-center p-4 border-t">
        <Button type="button" variant="outline" className="text-red-500 hover:text-red-600 border-red-200 hover:border-red-300 hover:bg-red-50" onClick={() => {
        if (onClose) {
          onClose();
        }
      }}>
