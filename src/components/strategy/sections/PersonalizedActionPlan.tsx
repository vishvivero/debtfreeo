    ...
    <>
      ...
      <Dialog open={showDueDateDialog} onOpenChange={setShowDueDateDialog}>
        <DialogContent className="max-w-2xl">
          <DialogHeader>
            <DialogTitle>Payment Due Dates</DialogTitle>
          </DialogHeader>
          <div className="space-y-4">
            <p className="text-sm text-gray-600">
              Keep track of all your payment due dates to avoid late fees and credit score damage.
            </p>
            <div className="border rounded-xl overflow-hidden">
              <table className="w-full text-sm">
                <thead className="bg-gray-100">
                  <tr>
                    <th className="px-4 py-3 text-left font-medium text-gray-700">Debt Name</th>
                    <th className="px-4 py-3 text-left font-medium text-gray-700">Due Date</th>
                    <th className="px-4 py-3 text-left font-medium text-gray-700">Payment Amount</th>
                  </tr>
                </thead>
      ...
      <Dialog open={showHighInterestDialog} onOpenChange={setShowHighInterestDialog}>
        <DialogContent className="max-w-2xl">
          ...
          <div className="space-y-4">
            <p className="text-sm text-gray-600">
              These debts have interest rates above 15%. Focusing on paying these off first will save you the most money long-term.
            </p>
            {highInterestDebts.length > 0 ? (
              <div className="border rounded-xl overflow-hidden">
                <table className="w-full text-sm">
                  <thead className="bg-gray-100">
                    <tr>
                      <th className="px-4 py-3 text-left font-medium text-gray-700">Debt Name</th>
                      <th className="px-4 py-3 text-left font-medium text-gray-700">Interest Rate</th>
                      <th className="px-4 py-3 text-left font-medium text-gray-700">Balance</th>
                      <th className="px-4 py-3 text-left font-medium text-gray-700">Annual Interest Cost</th>
                    </tr>
                  </thead>
                  <tbody className="divide-y">
                    {highInterestDebts.map((debt) => {
                      const annualInterest = Math.round(debt.balance * (debt.interest_rate / 100));
                      return (
                        <tr key={debt.id} className="hover:bg-gray-50">
                          <td className="px-4 py-3">{debt.name}</td>
                          <td className="px-4 py-3 text-red-600 font-medium">{debt.interest_rate}%</td>
                          <td className="px-4 py-3">
                            {debt.currency_symbol}{debt.balance.toLocaleString()}
                          </td>
                          <td className="px-4 py-3 text-red-600">
                            {debt.currency_symbol}{annualInterest.toLocaleString()}/year
                          </td>
                        </tr>
                      );
                    })}
              ...
              <div className="bg-blue-50 p-4 rounded-lg border border-blue-200 text-blue-800">
                You don't have any debts with interest rates above 15%. That's great! Continue making regular payments on your existing debts.
              </div>
            )}
            
            <div className="bg-amber-50 p-4 rounded-lg border border-amber-200">
              <h4 className="font-medium text-amber-800 mb-2">Why prioritize high-interest debt?</h4>
              <p className="text-sm text-amber-700">
                For every $1,000 in debt at 20% interest, you pay $200 per year just in interest. Reducing the same debt at only 5% interest would cost you just $50 per year - that's a $150 difference that could go toward paying off your principal balance faster.
              </p>
            </div>
